...
def combine_canvases(
    input_files, output_file, marker_styles_func, legend_text, log_x=False, log_y=False
):
    ...
    for canvas_name in [
        ...
        if marker_styles_func == momentum_styles:
            ...
            output_legend = ROOT.TLegend(
                0.5, 0.60, 1.15, 0.92
            )  # x1,y1,x2,y2 normalised coordinates in the current pad
                ...
                if legend:
                    ...
                    if input_legend:
                        for marker_idx in range(len(marker_styles)):
                            new_entry = (
                                input_legend.GetListOfPrimitives()
                                .At(marker_idx)
                                .Clone()
                            )
                            ...
                            legend_label = (
                                f"{new_entry.GetLabel()}{legend_text[input_file_idx]}\n"
                            )
        ...
        superposed_multigraph.Draw(
            "APE" if output_canvas.GetListOfPrimitives().GetSize() == 0 else "APEsame"
        )
        ...
        if marker_styles_func == theta_styles:
            ...
            superposed_multigraph.GetYaxis().SetRangeUser(
...
if __name__ == "__main__":
    ...
    combine_canvases(
        input_files, output_file, theta_styles, legend_text, log_x=False, log_y=True
    )
